 ...
 *
 * NO CLOUDFLARE CLEANUP IS NEEDED (it used to be: a stored OAuth grant had to
 * be revoked before the cascade dropped its ciphertext). Cloudflare
 * authorizations are now one-shot — minted, spent and revoked inside a single
 * provisioning request, never persisted — so a deleted account cannot leave
 * one behind. See src/lib/cfoauth.ts and migrations/0005_drop_cf_oauth.sql.
 */
