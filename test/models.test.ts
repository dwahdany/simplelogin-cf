...
import { createAlias, createContact, createMailbox, createUser } from "./fixtures";
...
describe("isPremium (pure precedence & grace)", () => {
  ...
  it("trial: premium while now < trial_end, not after", () => {
    expect(isPremium(emptyInputs({ trialEnd: toStr(addDays(NOW, 1)) }), NOW)).toBe(true);
    expect(isPremium(emptyInputs({ trialEnd: toStr(addDays(NOW, -1)) }), NOW)).toBe(false);
  });

  it("paddle active up to next_bill_date + 14 days (date granularity)", () => {
    // now - 14d = 2024-05-18 -> equal is still active
    expect(isPremium(emptyInputs({ paddle: paddle("2024-05-18") }), NOW)).toBe(true);
    // now - 15d = 2024-05-17 -> expired
    expect(isPremium(emptyInputs({ paddle: paddle("2024-05-17") }), NOW)).toBe(false);
    // future bill date -> active
    expect(isPremium(emptyInputs({ paddle: paddle("2024-07-01") }), NOW)).toBe(true);
  });

  it("cancelled paddle still counts within grace", () => {
    expect(isPremium(emptyInputs({ paddle: paddle("2024-05-20", true) }), NOW)).toBe(true);
  });

  it("apple valid while expires_date > now - 16 days", () => {
    const mk = (expires_date: string) => ({
      id: 1, created_at: toStr(NOW), updated_at: null, user_id: 1,
      expires_date, original_transaction_id: "t", receipt_data: "r",
      plan: "monthly" as const, product_id: null,
    });
    expect(isPremium(emptyInputs({ apple: mk("2024-05-16 12:00:01+00:00") }), NOW)).toBe(true);
    expect(isPremium(emptyInputs({ apple: mk("2024-05-16 12:00:00+00:00") }), NOW)).toBe(false);
  });

  it("manual/coinbase active while end_at > now", () => {
    const manual = {
      id: 1, created_at: toStr(NOW), updated_at: null, user_id: 1,
      end_at: toStr(addDays(NOW, 1)), comment: null, is_giveaway: 0,
    };
    ...
    const coinbase = {
      id: 1, created_at: toStr(NOW), updated_at: null, user_id: 1,
      end_at: toStr(addDays(NOW, 1)), code: null,
    };
  ...
  it("partner: lifetime or end_at within 14 days; excludable", () => {
    const lifetime = {
      id: 1, created_at: toStr(NOW), updated_at: null, partner_user_id: 1,
      end_at: null, lifetime: 1,
    };
    expect(isPremium(emptyInputs({ partner: lifetime }), NOW)).toBe(true);
    // excluded when include_partner=false
    expect(isPremium(emptyInputs({ partner: lifetime }), NOW, false)).toBe(false);

    const grace = { ...lifetime, lifetime: 0, end_at: toStr(addDays(NOW, -13)) };
...
describe("inTrial", () => {
  it("simple form: trial_end set and now < trial_end", () => {
    ...
    expect(inTrial({ trial_end: toStr(addDays(NOW, -1)) } as any, NOW)).toBe(false);
  });

  it("userInTrial is false when a subscription is active even if trial_end is future", async () => {
    const user = await createUser(env.DB, { trial_end: toStr(addDays(NOW, 5)) });
...
describe("maxAliasForFreeAccount", () => {
  it("defaults to MAX_NB_EMAIL_FREE_PLAN, 15 for the old-limit flag", () => {
    ...
    expect(maxAliasForFreeAccount({ flags: 0 } as any, { MAX_NB_EMAIL_FREE_PLAN: "" } as any)).toBe(5);
  });
});
...
describe("canCreateNewAlias", () => {
  it("free user: allowed under the limit, blocked at it; trashed excluded", async () => {
    ...
    await createAlias(env.DB, user.id, user.default_mailbox_id!, { delete_on: toStr(addDays(NOW, 30)) });
    ...
    await env.DB.prepare("UPDATE alias SET delete_on = ?1 WHERE id = ?2")
      .bind(toStr(addDays(NOW, 30)), row!.id).run();
    expect(await canCreateNewAlias(env.DB, testEnv, user, NOW)).toBe(true);
  });
  ...
  it("premium user is always allowed; disabled/inactive blocked", async () => {
    ...
    const inactive = await createUser(env.DB, { trial_end: null, delete_on: toStr(addDays(NOW, 5)) });
...
describe("availableSlEmail", () => {
  it("false if used by an alias, a contact reply_email, or a deleted_alias", async () => {
    ...
    const alias = await createAlias(env.DB, user.id, user.default_mailbox_id!, { email: "taken@sl.test" });
    expect(await availableSlEmail(env.DB, "taken@sl.test")).toBe(false);

    await createContact(env.DB, user.id, alias.id, { reply_email: "reply-used@sl.test" });
    expect(await availableSlEmail(env.DB, "reply-used@sl.test")).toBe(false);

    await env.DB.prepare("INSERT INTO deleted_alias (email, reason) VALUES (?1, 0)").bind("gone@sl.test").run();
...
describe("getSLDomains", () => {
  it("excludes hidden and premium_only for free users, ordered by 'order'", async () => {
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO public_domain (domain, premium_only, hidden, "order") VALUES ('b.test',0,0,2)`),
      env.DB.prepare(`INSERT INTO public_domain (domain, premium_only, hidden, "order") VALUES ('a.test',0,0,1)`),
      env.DB.prepare(`INSERT INTO public_domain (domain, premium_only, hidden, "order") VALUES ('premium.test',1,0,3)`),
      env.DB.prepare(`INSERT INTO public_domain (domain, premium_only, hidden, "order") VALUES ('hidden.test',0,1,4)`),
    ]);
    const free = await createUser(env.DB, { trial_end: null });
    const freeDomains = (await getSLDomains(env.DB, free, env, NOW)).map((d) => d.domain);
    expect(freeDomains).toEqual(["a.test", "b.test"]);

    const premium = await createUser(env.DB, { trial_end: null, lifetime: 1 });
    const premiumDomains = (await getSLDomains(env.DB, premium, env, NOW)).map((d) => d.domain);
...
describe("defaultRandomAliasDomain", () => {
  it("uses a verified custom domain, else public domain, else FIRST_ALIAS_DOMAIN", async () => {
    const testEnv = { ...env, FIRST_ALIAS_DOMAIN: "first.test", EMAIL_DOMAIN: "sl.test" } as any;
    const user = await createUser(env.DB, { trial_end: null });
    expect(await defaultRandomAliasDomain(env.DB, user, testEnv, NOW)).toBe("first.test");

    const cd = await env.DB.prepare(
      "INSERT INTO custom_domain (user_id, domain, verified) VALUES (?1,'mydomain.test',1) RETURNING id",
    ).bind(user.id).first<{ id: number }>();
    await env.DB.prepare("UPDATE users SET default_alias_custom_domain_id = ?1 WHERE id = ?2")
      .bind(cd!.id, user.id).run();
    const withCd = { ...user, default_alias_custom_domain_id: cd!.id };
    expect(await defaultRandomAliasDomain(env.DB, withCd, testEnv, NOW)).toBe("mydomain.test");
    ...
    await env.DB.prepare("UPDATE users SET default_alias_public_domain_id = ?1 WHERE id = ?2")
      .bind(pd!.id, pubUser.id).run();
    const withPub = { ...pubUser, default_alias_public_domain_id: pd!.id };
    expect(await defaultRandomAliasDomain(env.DB, withPub, testEnv, NOW)).toBe("first.test");
  });
});
...
describe("query helpers", () => {
  it("aliasMailboxIds returns primary first then extras; userVerifiedMailboxes is verified+sorted", async () => {
    ...
    const unverified = await createMailbox(env.DB, user.id, "unv@mb.test", { verified: 0 });
    const alias = await createAlias(env.DB, user.id, mbZ.id);
    await env.DB.prepare("INSERT INTO alias_mailbox (alias_id, mailbox_id) VALUES (?1,?2)").bind(alias.id, mbA.id).run();

    expect(await aliasMailboxIds(env.DB, alias.id)).toEqual([mbZ.id, mbA.id]);

    const verified = (await userVerifiedMailboxes(env.DB, user.id)).map((m) => m.email);
