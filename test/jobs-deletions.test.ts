...
import { setCfOauthFetch } from "../src/lib/cfoauth";
...
describe("delete-account job", () => {
  ...
  it("has no Cloudflare cleanup left to do: nothing was ever stored", async () => {
    // This used to revoke a stored OAuth grant before the users-row cascade
    // dropped its ciphertext. Cloudflare authorizations are now one-shot —
    // minted, spent and revoked inside a single provisioning request — so a
    // deleted account cannot leave one behind, the grant table is gone
    // (migrations/0005_drop_cf_oauth.sql), and account deletion must not talk
    // to dash.cloudflare.com at all.
    ...
    try {
      ...
      const tables = await db
        .prepare(
          "SELECT COUNT(*) AS n FROM sqlite_master WHERE type = 'table' AND name = 'cf_oauth_token'",
        )
        .first<{ n: number }>();
